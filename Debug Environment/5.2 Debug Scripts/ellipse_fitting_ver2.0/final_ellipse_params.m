function [ra_, rb_, centroid_, ang_, area_, high_low_rat_, dist_, xnew_, ynew_] = final_ellipse_params(ra, rb, centroid, ang, img, x_earth, y_earth, imin, jmin, LS_USA, deg2met)
IM = img < 150;
[y_bel, x_bel] = find(IM);
[y_abv, x_abv] = find(~IM);
...
if isempty(ra) == 0
    ...
    for k = 1:length(ra)
        ...
        id_non0 = find(~all(P_in_bel(:,:,k) == 0,2));
        ...
        yrow = round(y_centroid(k), 0) + min(imin) - 1;
        xcol = round(x_centroid(k), 0) + min(jmin) - 1;
        ME2 = LS_USA(:,:,2);
        ME3 = LS_USA(:,:,3);
