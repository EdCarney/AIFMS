function [ra_, rb_, centroid_, ang_, area_, high_low_rat_, dist_, xnew_, ynew_] = final_ellipse_params_noturn(ra, rb, centroid, ang, img, x_earth, y_earth, imin, jmin, LS_USA, deg2met, FGIF)
IM = img < 150;
[y_bel, x_bel] = find(IM);
[y_abv, x_abv] = find(~IM);
FGIF = [round(FGIF(:,1),0) round(FGIF(:,2),0)];
if isempty(ra) == 0
    ra_ = ra;
    rb_ = rb;
    centroid_ = centroid(:,:);
